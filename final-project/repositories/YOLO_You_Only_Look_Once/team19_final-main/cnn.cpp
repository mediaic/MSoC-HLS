...
// #pragma HLS ARRAY_RESHAPE variable=input_array complete dim=1
// #pragma HLS ARRAY_RESHAPE variable=input_array complete dim=2
// /*
...
//	ap_uint<6> T2R_bound = MIN(t2_local + T2R,OnChipIB_Height);
	...
	//YHC
	ap_uint<6> T2R_bound_idxmax = 64;
	// UCHAR TCol_idxmax = 53;
	// UCHAR TCol_idxmax = 256;

	// LOOP_cp_in2buf_row1:for(t2r = t2_local;t2r < 64 /*T2R_bound*/; t2r++){ //This will cause Pynq error
	LOOP_cp_in2buf_row1:for(t2r = t2_local;t2r < T2R_bound; t2r++){
#pragma HLS LOOP_TRIPCOUNT max=64
#pragma HLS PIPELINE II=1
	if(t2r<T2R_bound){
		// LOOP_cp_in2buf_row2:for(t3 = 0;t3 < 53; t3++)
		LOOP_cp_in2buf_row2:for(t3 = 0;t3 < TCol; t3++)
		{
#pragma HLS LOOP_TRIPCOUNT max=53
		if(t3<TCol){
			...
			if(IsRowPixel_t2r&&IsColPixel)
			{
				...
				if(NextInputFlag[0])
				{
					input_array[0][0] = input_memcpy_buffer_local[input_mmcpy_offset[0]];
					input_array[0][1] = input_memcpy_buffer_local[input_mmcpy_offset[0]] >> 16;
				...
				if(NextInputFlag[1])
				{
					input_array[1][0] = input_memcpy_buffer1_local[input_mmcpy_offset[1]];
					input_array[1][1] = input_memcpy_buffer1_local[input_mmcpy_offset[1]] >> 16;
				...
				if(NextInputFlag[2])
				{
					input_array[2][0] = input_memcpy_buffer2_local[input_mmcpy_offset[2]];
					input_array[2][1] = input_memcpy_buffer2_local[input_mmcpy_offset[2]] >> 16;
				...
				if(NextInputFlag[3])
				{
					input_array[3][0] = input_memcpy_buffer3_local[input_mmcpy_offset[3]];
					input_array[3][1] = input_memcpy_buffer3_local[input_mmcpy_offset[3]] >> 16;
...
#pragma HLS ARRAY_PARTITION variable=ouput_array1 complete dim=1
	...
	/*
	static int output_buffer_local [Tc];
	static int output_buffer1_local[Tc];
#pragma HLS ARRAY_RESHAPE variable=output_buffer_local  complete dim=1
#pragma HLS ARRAY_RESHAPE variable=output_buffer1_local complete dim=1
#pragma HLS RESOURCE variable=output_buffer_local  core=RAM_1P_BRAM
#pragma HLS RESOURCE variable=output_buffer1_local core=RAM_1P_BRAM
	for(int ii=0;ii<Tc;ii++){
#pragma HLS PIPELINE
		output_buffer_local [ii]	= output_buffer[tm][tr][ii];
		output_buffer1_local[ii]	= output_buffer[tm + (Tm>>1)][tr][ii];
	}
	*/
	...
	loop1_outputpixel2buf:for(tm_count = 0;tm_count < TM_LOOP;tm_count++,tm++,tr = 0){
#pragma HLS LOOP_TRIPCOUNT max=2
		loop2_outputpixel2buf:for(tr_count = 0;tr_count < TR_LOOP;tr_count++,tr++){
#pragma HLS LOOP_TRIPCOUNT max=13		
#pragma HLS PIPELINE
			loop3_outputpixel2buf:for(tc = 0;tc < TC_MIN_5b;tc++)
			// loop3_outputpixel2buf:for(tc = 0;tc < 32;tc++)
			{
#pragma HLS LOOP_TRIPCOUNT max=32	
// #pragma HLS PIPELINE
			if(tc<TC_MIN_5b){
				int tmp = output_buffer[tm][tr][tc];
				int tmp1 = output_buffer[tm + Tm/2][tr][tc];
				// int tmp = output_buffer_local[tc];
				// int tmp1 = output_buffer1_local[tc];
				if(IsNL&&tmp<0)
				{
...
#pragma HLS ARRAY_PARTITION variable=weight_buffer1 complete dim=2
	...
	if(LayerType==0)
	{
		...
		{
#pragma HLS LOOP_TRIPCOUNT max=320
			if(pingpong == 1)
			{
...
void copy_beta(short beta_buffer[MAX_BETA_LENGTH],ap_uint<32> *Beta,const int OFM_NUM,const int BetaQ)
{
	static ap_uint<32> beta_tmp[MAX_BETA_LENGTH/2];
	int NUM = (OFM_NUM+1)>>1; //OFM_NUM max=1024, NUM max=512
	memcpy(beta_tmp,(ap_uint<32> *)Beta,NUM*sizeof(ap_uint<32>));
	int x;
	for(x = 0;x < NUM;x++)
	{
...
void YOLO2_FPGA(int *Input,int *Input1,int *Input2,int *Input3,int *Output,int *Output1,int *Weight,ap_uint<32> *Beta,const int InFM_num,const int OutFM_num,
							  const int Kernel_size,const int Kernel_stride,
...
#pragma HLS INTERFACE m_axi depth=512 port=Input   offset=slave bundle=DATA_BUS1 num_read_outstanding=1 num_write_outstanding=1 max_read_burst_length=256 max_write_burst_length=256
#pragma HLS INTERFACE m_axi depth=512 port=Input1  offset=slave bundle=DATA_BUS2 num_read_outstanding=1 num_write_outstanding=1 max_read_burst_length=256 max_write_burst_length=256
#pragma HLS INTERFACE m_axi depth=512 port=Input2  offset=slave bundle=DATA_BUS3 num_read_outstanding=1 max_read_burst_length=256
#pragma HLS INTERFACE m_axi depth=512 port=Input3  offset=slave bundle=DATA_BUS4 num_read_outstanding=1 max_read_burst_length=256
#pragma HLS INTERFACE m_axi depth=512 port=Output  offset=slave bundle=DATA_BUS1 num_read_outstanding=1 num_write_outstanding=1 max_read_burst_length=256 max_write_burst_length=256
...
#pragma HLS INTERFACE m_axi depth=512 port=Weight  offset=slave bundle=DATA_BUS5 num_read_outstanding=1 max_read_burst_length=256
#pragma HLS INTERFACE m_axi depth=512 port=Beta    offset=slave bundle=DATA_BUS5 num_read_outstanding=1 max_read_burst_length=256
...
#pragma HLS LOOP_TRIPCOUNT max=16
			...
			{
#pragma HLS LOOP_TRIPCOUNT max=34 //32+2
