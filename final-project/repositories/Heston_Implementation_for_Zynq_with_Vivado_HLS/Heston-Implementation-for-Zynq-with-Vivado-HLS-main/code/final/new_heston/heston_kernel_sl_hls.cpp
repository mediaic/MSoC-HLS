	...
	for (uint32_t block = 0; block < path_cnt; block += BLOCK_SIZE) {
		for (uint32_t step = 0; step != step_cnt; ++step) {
			...
			for (uint32_t i = 0; i != BLOCK_SIZE; ++i) {
				...
				calc_t tmp_1 = (double_riskless_rate - max_vola) * half_step_size;
				calc_t tmp_2 = l_state.stock + tmp_1;
				#pragma HLS RESOURCE variable=tmp_2 core=FAddSub_nodsp

				n_state.stock = tmp_2 + sqrt_step_size * sqrt_vola *
							gaussian_rn1.read();
				...
				#pragma HLS RESOURCE variable=barrier_correction core=FMul_nodsp
