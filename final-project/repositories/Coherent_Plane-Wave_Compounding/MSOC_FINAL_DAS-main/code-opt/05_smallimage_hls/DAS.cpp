...
#pragma HLS RESOURCE variable=extended_apod_func_x core=DSP_Macro
	float z_sample;
	float tx_sample;
	...
	for (x_pos = 1; x_pos <= 16; x_pos++)
			    {
			    	...
			    	int IIR_count = 0;
			    	for (depth = 0; depth < Datalength_core; depth++)
			    	{
                    #pragma HLS PIPELINE
			    		snapshot_sum = 0;
			    		...
			    		depth_temp = (int)(depth)%7;
			    		buffer_real = snapshot_sum*demod_coeff[depth_temp].real;
			    	    buffer_imag = snapshot_sum*demod_coeff[depth_temp].imag;
			    	    IIR_pipe(buffer_real, buffer_imag, &IIR_buffer_real, &IIR_buffer_imag, x_pos);

			    	    int block_x_pos = x_pos-1;
			    	    depth_temp = (int)(depth)/8;
			    	    if (IIR_count == 7){
                            #pragma HLS PIPELINE
			    	    	fp_IQ_acc IQ_acc_real = IQ_acc_block_real[depth_temp][block_x_pos];
			    	    	fp_IQ_acc IIR_real =  IIR_buffer_real;
			    	    	fp_IQ_acc IQ_acc_imag = IQ_acc_block_imag[depth_temp][block_x_pos];
			    	    	fp_IQ_acc IIR_imag =  IIR_buffer_imag;
			    	        IQ_acc_block_real[depth_temp][block_x_pos] = IQ_acc_real + IIR_real;
			    	    	IQ_acc_block_imag[depth_temp][block_x_pos] = IQ_acc_imag + IIR_imag;
